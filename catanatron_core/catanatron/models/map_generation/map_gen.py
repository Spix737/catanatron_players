...
file_path = (
    "catanatron_core/catanatron/models/map_generation/catan_board_idp_theory.idp"
)
map_count = 0
...
def generate_map():
    ...
    with open(
        "catanatron_core/catanatron/models/map_generation/map_count_no_balance.txt", "r"
    ) as count_file:
    ...
    for model in model_expand(T, S, max=1):
        ...
        coordinate_tile_dict = {
            (int(x), int(y)): tile_type for x, y, tile_type in matches
        }
        matches = re.findall(pattern, tile_token)
        coordinate_token_dict = {
            (int(x), int(y)): tile_token for x, y, tile_token in matches
        }
    ...
    with open(file_path, "w") as file:
        ...
        for axial_coord, tile_value in coordinate_tile_dict.items():
            if index == 0:
                new_map_constraint += (
                    "~(tile_type" + str(axial_coord) + " = " + tile_value + ""
                )
                index += 1
            else:
                new_map_constraint += (
        ...
        for axial_coord, token_value in coordinate_token_dict.items():
            new_map_constraint += (
                " & tile_token" + str(axial_coord) + " = " + token_value + ""
            )
        new_map_constraint += ").\n"
    ...
    updated_content = (
        sections["vocabulary"] + sections["theory"] + sections["structure"]
    )
    with open(file_path, "w") as file:
        file.write(updated_content)
    rest_of_content += "tiles:" + str(tiles) + "\n" + "tokens:" + str(tokens) + "\n"
    with open(
        "catanatron_core/catanatron/models/map_generation/map_count_no_balance.txt", "w"
    ) as count_file:
