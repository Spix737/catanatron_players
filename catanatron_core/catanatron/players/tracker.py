    ...
    def update_opponent_resources(self, state, action):
        ...
        if action.action_type == ActionType.ROLL:
            ...
            try:
                if action.value is not None:
                    ...
                    for resource_index, quantity in enumerate(discard_deck):
                        ...
                        self.assumed_resources[action.color][resource] = max(
                            0, available - quantity
                        )

                        if available < quantity:
                            self.assumed_resources[action.color][UNKNOWN] -= (
                                quantity - available
                            )
                            for i in range(quantity - available):
                                self.assumed_resources[action.color][
                                    "unknown_list"
                                ].remove(resource)
                else:
                    print(
                        "No resources passed to discard from assumed\nThis could be problematic.................!"
                    )
            ...
            try:
                ...
                for victim in self.assumed_resources:
                    if victim != action.color:
                        ...
                        self.assumed_resources[action.color][
                            stolen
                        ] += self.assumed_resources[victim][stolen]
                        self.total_resources_gained[
                            action.color
                        ] += self.assumed_resources[victim][stolen]
                        self.total_resources_lost[victim] += self.assumed_resources[
                            victim
                        ][stolen]
                        # reset their known resource count to 0
                        self.assumed_resources[victim][stolen] = 0
                        count_to_take = list(
                            self.assumed_resources[victim]["unknown_list"]
                        ).count(stolen)

                        # remove all instances of stolen from the u list and from unknown count
                        self.assumed_resources[victim]["unknown_list"] = list(
                            filter(
                                lambda a: a != stolen,
                                self.assumed_resources[victim]["unknown_list"],
                            )
                ...
                if victim != None and robbed_resource != None:
                    ...
                    if action.color == self.color or victim == self.color:
                        ...
                        if self.assumed_resources[victim][robbed_resource] > 0:
                            ...
                            self.assumed_resources[victim]["unknown_list"].remove(
                                robbed_resource
                            )
                    # if the player isn't involved, there are unknowns
                        ...
                        self.assumed_resources[action.color]["unknown_list"].append(
                            robbed_resource
                        )
                        # remove the robbed resource from the victim's resources/unknowns
                        ...
                        self.assumed_resources[victim][UNKNOWN] += (
                            len(possibly_stolen) - 1
                        )
                        self.assumed_resources[victim]["unknown_list"].extend(
                            possibly_stolen
                        )
            ...
            for resource_quantity, resource_index in offered:
                ...
                if (
                    requester_assumed_resources[RESOURCES[resource_index]]
                    < resource_quantity
                ):
                    ...
                    for i in range(diff):
                        requester_assumed_resources["unknown_list"].remove(
            ...
            offered = trade_action_value[:5]
            if (
                self.assumed_resources[action.color][RESOURCES[resource_index]]
                < resource_quantity
            ):
                # take from unknown and unknown list,
                diff = (
            ...
            for resource_quantity, resource_index in offered:
                for i in range(resource_quantity):
                    if (
                        ...
                        self.assumed_resources[action.color][UNKNOWN] -= 1
                        self.assumed_resources[action.color]["unknown_list"].remove(
                            RESOURCES[resource_index]
                        )
            ...
            try:
                ...
                for i in range(self.assumed_resources[action.color][UNKNOWN]):
                    if (
                        self.assumed_resources[action.color]["unknown_list"][i]
                        == giving[0]
                    ):
                        givingrez += 1
