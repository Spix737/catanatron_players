...
def save_to_csv(
    file_path, game_id, game_data, turn_count, players, epsilon, average_loss
):
...
class PrioritizedReplayBuffer:
    ...
    def store(self, state, action, reward, state_, done):
        ...
        self.priority_memory[index] = (
            self.priority_memory.max() if self.mem_cntr > 0 else 1.0
        )
        self.mem_cntr += 1
    ...
    def update_priorities(self, indices, priorities):
        ...
        if not isinstance(priorities, np.ndarray):
            priorities = np.array(
                [priorities]
            )  # Convert single priority value to an array
...
class DQNAgent:
    ...
    def choose_action(self, observation):
        ...
        if np.random.random() <= self.epsilon:
            ...
            state = torch.tensor([processed_observation], dtype=torch.float32).to(
    ...
    def learn(self):
        ...
        loss = (
            loss * torch.tensor(weights, dtype=torch.float32).to(self.policy_net.device)
        ).mean()
        loss.backward()
        self.optimizer.step()
        ...
        loss_value = (
            loss.item() + 1e-5
        )  # Convert loss to Python scalar and add a small offset
        self.memory.update_priorities(
            indices, np.array([loss_value])
        )  # Ensure the priority is passed as an array
...
def game_end_collector(dqn_agent):
    ...
    settlements = len(
        get_player_buildings(env.unwrapped.game.state, my_color, SETTLEMENT)
    )
    ...
    total_resources_gained = env.unwrapped.my_card_counter.total_resources_gained[
        my_color
    ]
    amount_of_resources_used = env.unwrapped.my_card_counter.total_resources_used[
        my_color
    ]
    seven_robbers_moved = (
        env.unwrapped.my_card_counter.total_robbers_moved[my_color]
        - env.unwrapped.game.state.player_state[f"{key}_PLAYED_KNIGHT"]
    )
    knights_and_robbers_moved = env.unwrapped.my_card_counter.total_robbers_moved[
        my_color
    ]
    ...
    total_resources_discarded = env.unwrapped.my_card_counter.total_resources_discarded[
        my_color
    ]
    ...
    dev_cards_held = {
        ...
        "ROAD_BUILDING": env.unwrapped.game.state.player_state[
            f"{key}_ROAD_BUILDING_IN_HAND"
        ],
        "YEAR_OF_PLENTY": env.unwrapped.game.state.player_state[
    ...
    dev_cards_used = {
        ...
        "ROAD_BUILDING": env.unwrapped.game.state.player_state[
            f"{key}_PLAYED_ROAD_BUILDING"
        ],
        "YEAR_OF_PLENTY": env.unwrapped.game.state.player_state[
            f"{key}_PLAYED_YEAR_OF_PLENTY"
        ],
    }
...
if __name__ == "__main__":
    ...
    for i in range(n_games):
        ...
        key = player_key(env.unwrapped.game.state, Color.BLUE)
        end_points = env.unwrapped.game.state.player_state[
        ...
        if (i) % 600 == 0:  # Checkpoint every 1000 episodes
            checkpoint_filename = (
                f"model_data_simplerinitbuild/dqn_model_checkpoint_{i}.pth"
            )
            torch.save(agent.policy_net.state_dict(), checkpoint_filename)
            torch.save(
                agent.optimizer.state_dict(),
                f"model_data_simplerinitbuild/dqn_optimizer_checkpoint_{i}.pth",
            )
        ...
        save_to_csv(
            file_path, game_id, game_stats, turn_count, players, agent.epsilon, avg_loss
        )
        game_id += 1
    ...
    torch.save(
        agent.policy_net.state_dict(), "model_data_simplerinitbuild/dqn_model_final.pth"
    )
    torch.save(
        agent.optimizer.state_dict(),
        "model_data_simplerinitbuild/dqn_optimizer_final.pth",
    )
