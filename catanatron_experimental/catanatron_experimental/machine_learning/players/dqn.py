...
class DQNAgent:
    def __init__(
        ...
        self.model = DQN(
            state_size, n_actions, self.learning_rate
...
def train_dqn_agent(gym_env, episodes=1200):
    ...
    for e in range(episodes):
        ...
        if episode_rewards > best_total_reward:
            ...
            print(
                f"New best model saved with reward: {best_total_reward} at episode: {e+1}"
            )
...
def train_dqn_agent_multi_env(envs, episodes=3, checkpoint_interval=1000):
    ...
    agent = DQNAgent(
        env=None, my_color=Color.BLUE, state_size=state_size, action_size=action_size
    )  # Modified to pass None as the initial environment
    batch_size = 32
    ...
    for e in range(episodes):
        ...
        for env_name in envs:
            ...
            while not done:
                ...
                if done:
                    print(
                        f"GAME END: episode: {e}/{episodes}, env: {env_name}, score: {episode_rewards}, e: {agent.epsilon:.2}"
                    )
                    break
                if len(agent.memory) > batch_size:
                    agent.replay(batch_size)
            ...
            if (
                e + 1
            ) % checkpoint_interval == 0 or e == 0:  # Also save on the first episode
            ...
            if episode_rewards > best_total_reward:
                ...
                torch.save(agent.model.state_dict(), best_model_filename)
                print(
                    f"New best model saved with reward: {best_total_reward} at episode: {e+1}, env: {env_name}"
                )
...
if __name__ == "__main__":
    ...
    for i in range(n_games):
        ...
        if score >= best_total_reward and end_points >= best_end_points:
            ...
            best_model_filename = (
                f"dqn_best_model_{best_total_reward}_{best_end_points}.pth"
            )
            torch.save(agent.model.state_dict(), best_model_filename)
            print(
                f"New best model saved with reward: {best_total_reward} at episode: {best_total_reward}"
            )
