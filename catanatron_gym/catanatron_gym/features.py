...
ACTIONS_ARRAY = [(Color.BLUE, ActionType.ROLL, None),
...
def available_actions_features(game: Game, p0_color: Color):
    ...
    for i, color in iter_players(game.state.colors, p0_color):
        ...
        if color == p0_color:
            if not playable_actions or not any(action.color == p0_color for action in playable_actions):
                ...
                for action in ACTIONS_ARRAY:
                    ...
                    action_value = action[2]
                    simple_action_type = str(action_type).replace("ActionType.", "")
                    features[f"P0_can_{simple_action_type}_{action_value}"] = 1 if action in playable_actions else 0
